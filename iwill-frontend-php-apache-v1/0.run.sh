...
ADD dcs/nginx.conf /etc/nginx/sites-available/default
ADD dcs/supervisord.conf /etc/supervisor/conf.d/supervisord.conf
ADD dcs/initial.sh /usr/bin/initial
...
# docker build -t dcsurani/iwillfrontend -f Dockerfile .

# docker stop hello && docker rm hello

# # docker run --name hello -d -p 80:80 -p 9000:9000 dcsurani/iwill
# docker run --name hello -d -p 8080:8080 dcsurani/iwillfrontend:260722.113313
